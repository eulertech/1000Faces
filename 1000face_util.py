...
def get_project ( pid ):
    ...
    str1 = "select * from public.projects where pid in ({})".format(','.join(p_id_str))
    print str1
    cursor.execute (str1)
    rows = cursor.fetchall()

    columns = ('pid','projectname','projectteam','projectowner', 'projectdescription', 'projectcategory')
    data = []
    for row in rows:
        data.append(dict(zip(columns, row)))
    return json.dumps(data)
...
def get_node (eid):
    ...
    response = {"node": data, "link": data1}

    return json.dumps(response)

if __name__ == "__main__":

    print get_employee([12126, 12127])
    print get_node([12126, 12127])
    print get_project([1, 2])